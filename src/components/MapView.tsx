...
export function MapView({ aoiStatus, aoiVertices, onMapReady }: MapViewProps) {
  ...
  useEffect(() => {
    ...
    // Use the existing createMap() from src/map.ts which handles
    // PMTiles protocol, OpenFreeMap basemap, and proper attribution.
    createMap({
      ...
      {aoiStatus === 'drawing' && (
        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-10">
          <Badge className="gap-2 text-sm px-4 py-2 shadow-lg">
            ...
            {aoiVertices > 0 && ` (${aoiVertices} vertices)`}
          </Badge>
        </div>
      )}
