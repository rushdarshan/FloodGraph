...
import { PyodideStatus as PyodideStatusType } from '../../App';

interface PyodideStatusProps {
  status: PyodideStatusType;
  progress: number;
}

export function PyodideStatus({ status, progress }: PyodideStatusProps) {
  if (status === 'ready') {
    return (
      <Alert className="border-green-500/50 bg-green-500/10">
        <CheckCircle2 className="h-4 w-4 text-green-600 dark:text-green-400" />
        <AlertTitle className="text-sm text-green-600 dark:text-green-400">
          Pyodide Ready
        </AlertTitle>
